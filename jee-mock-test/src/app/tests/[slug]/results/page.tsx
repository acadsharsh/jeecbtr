  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex items-center gap-3 mb-8">
        <Link href={`/tests/${params.slug}`}
          className="p-2 border-2 border-ink-900 hover:bg-ink-50 transition-colors">
          <ArrowLeft size={16} />
        </Link>
        <div>
            ...
            {Object.entries(topicScores).map(([topic, data]) => {
              ...
              return (
                <div key={topic} className="px-5 py-4 flex items-center gap-4">
                  ...
                  <span className="font-mono text-xs text-ink-600 w-20 text-right">
              ...
              <div key={q.id} className="px-5 py-5">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="font-body text-sm text-ink-900 mb-3 leading-relaxed">
                      <MathText text={q.question_text} />
                    </div>

                    {/* Diagram */}
                    {q.diagram_url && (
                      <div className="mb-3 border border-ink-200 inline-block p-1">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={q.diagram_url} alt="Diagram" className="max-h-40 object-contain" />
                      </div>
                    )}
                        ...
                        {q.options.map(opt => {
                          ...
                          return (
                            <div key={opt.label} className={cn(
                              "px-3 py-2 border text-xs flex items-start gap-2",
                              isCorrectOpt
                                ? "border-emerald-500 bg-emerald-50"
                                : isUserPick && !isCorrectOpt
                                ? "border-crimson-500 bg-red-50"
                    ...
                    {q.explanation && (
                      <div className="mt-2 border-l-4 border-amber-500 pl-3 py-1.5 bg-amber-50">
